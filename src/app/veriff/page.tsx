...
// Visual config for different Veriff callback events/statuses
...
> = {
  success: {
    ...
    description: "Your identity verification documents have been successfully submitted to Veriff. The review process is now underway.",
  ...
  canceled: {
    ...
    description: "You have cancelled the identity verification process. You can return and complete it at any time.",
  ...
  error: {
    ...
    description: "An error occurred or the session expired during the verification process. Please try again.",
...
const defaultErrorConfig = {
  ...
  description: "An unexpected status or error response was received from Veriff. Please return to your dashboard.",
  gradient: "from-red-500/10 via-rose-500/5 to-transparent",
  iconColor: "text-red-500",
  accentColor: "border-red-500/30 bg-red-50/50 text-red-700",
  isSuccess: false,
...
function VeriffContent() {
  ...
  // Map Veriff status codes & query params to visual states
  let event = "success";
  if (
    statusParam === "canceled" ||
    statusParam === "abandoned" ||
    statusCodeParam === "9104"
  ) {
    event = "canceled";
  } else if (
    statusParam === "fail" ||
    statusParam === "error" ||
    ["9102", "9103"].includes(statusCodeParam)
  ) {
    event = "error";
  } else if (
    statusParam === "success" ||
    ["9001", "7002", "7001", "9121"].includes(statusCodeParam)
  ) {
    event = "success";
  } else if (!statusParam && !statusCodeParam) {
    // If no params are passed, default to success/submitted display
    event = "success";
  }

  const config = EVENT_CONFIG[event] ?? defaultErrorConfig;
      ...
      >
        ...
        <div className="w-full bg-[#F5F5F5] rounded-2xl p-4 mb-8 flex items-center justify-between border border-gray-100">
          <div className="flex items-center gap-3 text-left">
            ...
            <div>
              <p className="font-work-sans text-[11px] font-bold text-[#181D27] uppercase tracking-wider">AristoPay Verification</p>
              <p className="font-work-sans text-xs text-[#535862]">Secure identity check powered by Veriff</p>
            </div>
          </div>
        </div>
...
export default function VeriffPage() {
  return (
    <Suspense
      fallback={
