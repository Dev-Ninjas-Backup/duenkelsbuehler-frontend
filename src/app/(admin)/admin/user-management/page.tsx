  ...
  return (
    <div className="flex flex-col gap-6 h-[calc(100dvh-160px)]">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: 0.1 }}
        className="grid grid-cols-[40px_1fr_80px_1.5fr_80px] gap-0 bg-[#181D27] text-white rounded-xl px-6 py-4 items-center shrink-0">
        <span className="font-work-sans text-sm font-medium">Sl</span>
        <span className="font-work-sans text-sm font-medium">Name</span>
        <span className="font-work-sans text-sm font-medium text-center">Sign in</span>
        <span className="font-work-sans text-sm font-medium">Date & Time</span>
      ...
      <motion.div key={currentPage} variants={containerVariants} initial="hidden" animate="visible"
        ...
        {isLoading ? (
          ...
          <motion.div key={user.id} variants={rowVariants}
            className="grid grid-cols-[40px_1fr_80px_1.5fr_80px] gap-0 items-center bg-[#F9F9F9] rounded-2xl px-6 py-5 shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-work-sans text-sm font-bold text-[#181D27]">{user.name}</span>
                {user.isIdentityVerified ? (
                  <span className="flex items-center gap-1 font-work-sans text-[10px] sm:text-xs text-[#16A34A] bg-[#16A34A]/10 px-2 py-0.5 rounded-full font-semibold">
                    <Image src="/svg/crown.svg" alt="Verified" width={12} height={12} /> Verified
                  </span>
                ) : (
                  <span className="flex items-center gap-1 font-work-sans text-[10px] sm:text-xs text-red-500 bg-red-500/10 px-2 py-0.5 rounded-full font-semibold">
            ...
            <div className="flex justify-center">
              {user.PROVIDER === "GOOGLE" ? (
                ...
                <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="#414651" strokeWidth="1.5">
