...
export function AddBadgeModal({ isOpen, onClose, editingBadge }: Props) {
  ...
  useEffect(() => {
    if (editingBadge) {
      ...
      setTitle(""); setDescription(""); setImageUrl(""); setType("");
    }
  }, [editingBadge, isOpen]);

  const handleClose = () => { setTitle(""); setDescription(""); setImageUrl(""); setType(""); onClose(); };

  const canSubmit = title.trim() && description.trim() && imageUrl.trim() && type;
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" placeholder="https://example.com/badge.png" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-4 py-3 font-work-sans text-sm text-[#181D27] placeholder:text-gray-400 focus:outline-none focus:border-[#181D27] transition-colors" />
            </div>

            <motion.button whileTap={{ scale: 0.98 }} disabled={!canSubmit || isCreating || isUpdating}
              ...
              {isCreating || isUpdating ? "Saving..." : editingBadge ? "Save Changes" : "Create Badge"}
            </motion.button>
          </motion.div>
        </motion.div>
      )}
